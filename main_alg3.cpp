...
map<int,vector<int>> power_set_of_U;
int power_set_num = 0;
...
void printPowerSet(vector<int> const &S, vector<int> &set, int n)
{
    ...
    if (n == 0)
    {
        power_set_of_U[++power_set_num] = set;
        return;
    }
...
/*
Print the equation.lp for alg3
*/

void print_equations_powerset(ostream& equationsfile, vector<int> tau_s){
    // We should rank all tau in tau_s
	last_used_lambda = 0;
	equationsfile<<"maximize ";
    for(vector<int>::iterator vit = tau_s.begin(); vit != tau_s.end(); ++vit){
	    equationsfile<<"+eps"<<*vit;
    }
	equationsfile<<"\n\nst\n\n";
	for(vector<int>::iterator vit = unranked_eps.begin(); vit!=unranked_eps.end(); ++vit){
		if(std::find(tau_s.begin(), tau_s.end(), *vit) == tau_s.end()){ // if not in tau_s
	...
	for(vector<int>::iterator vit = unranked_eps.begin(); vit!=unranked_eps.end(); ++vit){
		if(std::find(tau_s.begin(), tau_s.end(), *vit) == tau_s.end()){
    ...
    for(vector<int>::iterator taus_it=tau_s.begin(); taus_it != tau_s.end(); ++taus_it){
        int tau_0 = *taus_it;
...
bool process_equations_output_powerset(vector<int> tau_s){
	vector<double> temp(nVariables+1,0);
	for(map<int,CFG_location*>::iterator it = label_map.begin();it!=label_map.end();++it){
		it->second->lexrsm.push_back(temp);
	}
	...
	if(line=="CPLEX> Variable Name           Solution Value"){
		...
		while(to_process>>temp1>>temp2){
			if(temp1.length()>3){
				if(temp1.substr(0,3)=="eps"){
                    int eps_id = stoi(temp1.substr(3));
                    if(std::find(tau_s.begin(), tau_s.end(), eps_id) != tau_s.end()){  // Only remove the transitions in tau_s
                        if(stoi(temp2) == 1){
                            ranked_eps.push_back(eps_id);
                        }
                    }
				}
				else if(temp1.substr(0,2)=="f_"){
	...
	dimension_used++;
    if(ranked_eps.size() == tau_s.size()){
        to_return = true;
    }
    if(to_return){ // Sucessfully ranked, remove equations
        for(vector<int>::iterator tau_it = tau_s.begin(); tau_it != tau_s.end(); ++tau_it){
            int eps_id=*tau_it;
            for(vector<int>::iterator vit = eps_map_eqs[eps_id].begin(); vit!=eps_map_eqs[eps_id].end();vit++){
                equations.erase(*vit);
            }
...
int main(){
	...
	int r,i;
	FILE *f;
	// f = fopen("./examples/rebuttal/geometric_exp.prob","r");
	// f = fopen("./examples/rebuttal/nested_loops.prob","r");
	// f = fopen("./examples/ForExperiments/alain.prob","r");
	...
	generate_equations();
    // findPowerSet(unranked_eps);
    // cout<<"Generate Power Set FINISHED!!!!" << endl;
    // cout << "We have " << unranked_eps.size() << "unranked transitions"<<endl;
    // cout<< "Power set size is: " << power_set_of_U.size() << endl;
	...
	for(;loop_counter<100;++loop_counter){	
		...
		if(state==false){
            ...
            findPowerSet(unranked_eps); // Generate powerset

            for(map<int,vector<int>>::iterator mit = power_set_of_U.begin(); mit!=power_set_of_U.end();mit++){
                if(mit->second.size()!= 0){ // Non-empty powersets
                    vector<int> unranked_transition_ids = mit->second;
                    equationsfile.open("files/equations.lp");
                    ostream* equation_output_file = &equationsfile;
                    print_equations_powerset(*equation_output_file,unranked_transition_ids);
                    equationsfile.close();
                    if(system("./files/script.sh")!=0){
			            cout<<"Something wrong with the script analysing equations"<<endl;
                    }
                    single_state = process_equations_output_powerset(unranked_transition_ids);
